...
async function getVehicles() {
  const { data, error } = await supabase.from('vehicles').select('*').order('total_score', { ascending: false })
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-[#F5F5F5]">
      <header className="border-b border-[#C9A84C]/30 p-6">
        ...
        <p className="text-gray-400 text-sm mt-1">世界名車 価値評価インデックス</p>
      </header>
      <div className="max-w-6xl mx-auto p-6">
        <h2 className="text-xl font-bold text-white mb-6">🏆 評価指数トップ {vehicles.length}</h2>
        <div className="grid gap-4">
          {vehicles.map((v: any, i: number) => (
            <div key={v.id} className="bg-[#1A1A1A] rounded-xl p-5 border border-gray-800 hover:border-[#C9A84C]/50 transition-all">
              <div className="flex items-start gap-4">
                <span className="text-2xl font-bold text-[#C9A84C] w-8 shrink-0">#{i + 1}</span>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-lg font-bold text-white">{v.name_jp}</h3>
                    {v.steering === 'right' ? <span className="text-xs bg-blue-900/50 text-blue-300 px-2 py-0.5 rounded-full">🔵 右ハンドル</span> : <span className="text-xs bg-orange-900/50 text-orange-300 px-2 py-0.5 rounded-full">🟠 左ハンドル</span>}
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-800 rounded-full h-2"><div className="bg-[#C9A84C] h-2 rounded-full" style={{ width: `${v.total_score}%` }} /></div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-800 rounded-full h-2"><div className="bg-blue-500 h-2 rounded-full" style={{ width: `${v.japan_total_score}%` }} /></div>
