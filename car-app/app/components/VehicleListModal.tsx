...
export default function VehicleListModal({ vehicles, totalCount }: { vehicles: Vehicle[], totalCount: number }) {
  ...
  vehicles.forEach(v => {
    ...
    else grouped[v.country] = [v]
  })

  const displayList = selectedCountry
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ fontSize: 10, color: '#4BA3D3', background: '#E8F4FD', padding: '2px 8px', borderRadius: 99, marginTop: 2 }}>
          タップで一覧 →
        </div>
      </button>
        ...
        >
          <div style={{
            ...
            <div style={{
              ...
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 900, color: '#2C3E50', margin: 0 }}>
                  🚗 登録車両一覧
                </h2>
                <p style={{ fontSize: 12, color: '#8BA4B5', margin: '4px 0 0' }}>
                  全{totalCount}台 · 7カ国のクラシックカー
                </p>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                style={{
                  background: '#F8FAFB', border: '2px solid #BAE0F7',
                  borderRadius: 12, width: 36, height: 36,
                  fontSize: 16, cursor: 'pointer', color: '#8BA4B5',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontWeight: 700,
                }}
              >
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: selectedCountry === null ? '#4BA3D3' : '#F0F9FF',
                  color: selectedCountry === null ? 'white' : '#8BA4B5',
                  transition: 'all 0.2s',
                }}
              >
                🌍 全て（{totalCount}台）
                ...
                return (
                  <button
                    ...
                    style={{
                      padding: '6px 14px', borderRadius: 99, fontSize: 12, fontWeight: 700,
                      cursor: 'pointer',
                      border: `2px solid ${selectedCountry === c ? cc.color : cc.border}`,
                      background: selectedCountry === c ? cc.color : cc.bg,
                      color: selectedCountry === c ? 'white' : cc.color,
                      transition: 'all 0.2s',
                    }}
                  >
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '16px 24px' }}>
              {selectedCountry && (
                <div style={{
                  ...
                  background: countryColors[selectedCountry]?.bg || '#F8FAFB',
                  borderRadius: 12,
                  display: 'flex', alignItems: 'center', gap: 10
                }}>
                  <span style={{ fontSize: 28 }}>{countryFlag[selectedCountry]}</span>
                  <div>
                    <p style={{ fontSize: 16, fontWeight: 900, color: countryColors[selectedCountry]?.color, margin: 0 }}>
                      {selectedCountry}の名車
                    </p>
                    <p style={{ fontSize: 12, color: '#8BA4B5', margin: 0 }}>
                      {grouped[selectedCountry]?.length || 0}台登録
                    </p>
                  </div>
                </div>
              )}
                    ...
                    return (
                      ...
                      >
                        <div style={{
                          ...
                          onMouseEnter={e => {
                            (e.currentTarget as HTMLDivElement).style.background = cc.bg
                            ;(e.currentTarget as HTMLDivElement).style.borderColor = cc.border
                          }}
                          onMouseLeave={e => {
                            (e.currentTarget as HTMLDivElement).style.background = '#FAFCFF'
                            ;(e.currentTarget as HTMLDivElement).style.borderColor = '#E8F4FD'
                          }}
                        >
                          {/* 順位 */}
                          <div style={{
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontSize: 14, fontWeight: 800, color: '#2C3E50', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {v.name_jp}
                            </p>
                            <p style={{ fontSize: 11, color: '#8BA4B5', margin: 0 }}>
                              {v.maker} · {v.year_start}〜{v.year_end}年
                            </p>
                          </div>
